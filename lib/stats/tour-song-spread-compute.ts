...
/** Matches `CategorySpread` in `song-spread-display` (kept here to avoid client imports). */
export interface TourSongSpreadCategory {
  category: string
  count: number
  canonid: number
  songs: string[]
...
/** Same shape as `TourSongSpread` props `shows` — used for stats year / all-time. */
export interface TourSongSpreadShowInput {
  show_id: string
...
/**
 * Song spread by category — identical logic to `components/dpro/tours/tour-song-spread.tsx`.
 */
export function computeTourSongSpreadFromShows(
  shows: TourSongSpreadShowInput[],
): TourSongSpreadCategory[] {
