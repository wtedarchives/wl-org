...
function sanitizeStoragePath(raw: string | null): string | null {
  if (!raw?.trim()) return null
  const path = raw.trim().replace(/^\/+/, "")
  if (!path || path.includes("..") || path.includes("\\")) return null
  if (!/^[a-zA-Z0-9_./-]+\.png$/i.test(path)) return null
  return path
}
...
serve(async (req) => {
  ...
  const storagePath = sanitizeStoragePath(reqUrl.searchParams.get("path"))
