      ...
      try {
        ...
        if (showIds.length > 0) {
          const { data: showRows, error: showErr } = await client
            .from("shows")
            .select(
              "show_id, show_date, show_venue_location, show_group, discography_display",
            )
            .in("show_id", showIds)
        ...
        const sourceLabels = buildDiscographySourceLabels(ordered, showsById)
