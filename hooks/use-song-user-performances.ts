...
export function useSongUserPerformances(
  open: boolean,
  ...
  useEffect(() => {
    ...
    async function fetchPerformances() {
      try {
        ...
        for (let i = 0; i < attendedShowIds.length; i += CHUNK_SIZE) {
          chunks.push(attendedShowIds.slice(i, i + CHUNK_SIZE))
        }
        ...
        console.error("Error fetching song user performances:", err)
    ...
    fetchPerformances()
  }, [open, songName, userId, attendedShowIds])
