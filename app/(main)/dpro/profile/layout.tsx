...
export default function ProfileLayout({
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.replace(`/login?from=${encodeURIComponent("/dpro/profile/overview")}`)
    }
  ...
  const handleShare = async () => {
    ...
    const url =
      typeof window !== "undefined"
        ? `${window.location.origin}/dpro/user/${user.id}`
        : `/dpro/user/${user.id}`
    try {
            ...
            <TabsList className="h-8 w-full flex-wrap justify-start">
              {TABS.map((tab) => (
                <TabsTrigger
                  ...
                  className="text-xs"
                >
                  <Link href={`/dpro/profile/${tab.slug}`}>{tab.label}</Link>
                </TabsTrigger>
              ))}
            </TabsList>
          </div>
